...
enum Instruction {
    ...
    CLRF   = 0x0180, // Lösche f (Opcode: 00 0001 1fff ffff)
    CLRW   = 0x0100, // Lösche W (Opcode: 00 0001 0xxx xxxx)
    ...
    MOVWF  = 0x0080, // Bewege W nach f (Opcode: 00 0000 1fff ffff)
    ...
    BCF    = 0x1000, // Bit Löschen in f (Opcode: 01 00bb bfff ffff)
    BSF    = 0x1400, // Bit Setzen in f (Opcode: 01 01bb bfff ffff)
    ...
    CLRWDT = 0x0064, // Watchdog-Timer löschen (Opcode: 00 0000 0110 0100)
    GOTO   = 0x2800, // Gehe zu Adresse (Opcode: 10 1kkk kkkk kkkk)
    IORLW  = 0x3800, // Inklusives ODER Literal mit W 
    MOVLW  = 0x3000, // Bewege Literal nach W (Opcode: 11 00xx kkkk kkkk)
    RETFIE = 0x0009, // Rückkehr vom Interrupt (Opcode: 00 0000 0000 1001)
    RETLW  = 0x3400, // Rückkehr mit Literal in W (Opcode: 11 01xx kkkk kkkk)
    RETURN = 0x0008, // Rückkehr vom Unterprogramm (Opcode: 00 0000 0000 1000)
    SLEEP  = 0x0063, // Wechsel in Standby-Modus (Opcode: 00 0000 0110 0011)
